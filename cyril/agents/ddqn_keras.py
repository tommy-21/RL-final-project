...
class Brain:
    ...
    def createModel(self):
        model = tf.keras.Sequential()
        model.add(tf.keras.layers.Dense(256, activation=tf.nn.relu))  # Augmentation de la taille de la première couche cachée
        model.add(tf.keras.layers.Dense(256, activation=tf.nn.relu))  # Augmentation de la taille de la deuxième couche cachée
        model.add(tf.keras.layers.Dense(self.NbrActions, activation=tf.nn.softmax))
        model.compile(loss = "mse", optimizer="adam")
